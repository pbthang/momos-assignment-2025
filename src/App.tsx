...
const transformDTOToDataRecord = (
  ...
  const getCheckbox = (): boolean => {
    ...
    return checkboxProperty?.type === "checkbox" && checkboxProperty.checkbox;
  };
  const getDate = (): Date => {
  ...
  const getNumber = (): number => {
    ...
    if (numberProperty?.type === "number" && numberProperty.number !== null) {
  ...
  const getSelect = (): { name: string; color: string } | null => {
    ...
    if (selectProperty?.type === "select" && selectProperty.select !== null) {
      return {
  ...
  const getStatus = (): { name: string; color: string } | null => {
    ...
    if (statusProperty?.type === "status" && statusProperty.status !== null) {
      return {
